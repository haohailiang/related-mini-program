...
function showRelated() {
  ...
  let allFiles = []
  const curRelativePath = vscode.workspace.asRelativePath(currentFilename)

  // mini program indentity
...
// console.info('entries: ', entries);
  const items = allFiles.map(generateQuickPickItem);

  if (items.length) {
    const placeholderText = `文件${vscode.workspace.asRelativePath(
          currentFilename
    )}的相关联文件`;
  ...
  window
    ...
    input.placeholder = "相关文件没有搜索到, 请检索所有文件!";
    input.show();
